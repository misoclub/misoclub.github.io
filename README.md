...
1. **アイコン**を `assets/apps/<アプリ>.png`（正方形・256px 以上）に置く
2. **`apps.html`** の `<article class="app-card">` を複製し、アイコンのパス・アプリ名・説明・各ストアの URL（`href="#"`）を差し替え
3. **DLページ**（任意）：`dl/` のファイルを複製し、アプリ名と各ストアの URL（`href="#"`）を差し替え
4. **プライバシーポリシー**（必要なら）：下記の手順で追加

ストアバッジは App Store / Google Play とも公式バッジ画像を使い、両者を**同じ高さ・横並び**で表示します（`apps.html` のカード内、および `dl/` の各ページ）。
...
- [ ] ストアURL（App Store / Google Play）を差し替え（`apps.html` のカード内バッジ・`dl/` 各ページ。現在は `href="#"`）
