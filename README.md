...
```
.
├── index.html              # ランディングページ（トップ。今は最小限）
├── apps/
...
4. **`apps/index.html`** の `<article class="app-card">` を 1 つ複製し、アイコンのパス（`./<アプリ>/icon.png`）・アプリ名・説明・各ストアの URL を差し替え
...
- [ ] トップページ（`index.html`）の内容（今は最小限のプレースホルダ）
